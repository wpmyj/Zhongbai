...
{
    public partial class FrmAlterBitumenCostCount : BaseForm
    {

        private ArrayList treefields = new ArrayList();
        public FrmAlterBitumenCostCount()
        {
            InitializeComponent();
        }

        AuditingAction auditingActionAlterBitumen = new AuditingAction("id", "alterBitumenExpenditure");
        AuditingAction auditingActionAlterBitumenDetach = new AuditingAction("id", "alterBitumenWaitDetachExpenditure");
        ...
        private void btnExpenditureAdd_Click(object sender, EventArgs e)
        {
            ...
            FrmAlterBitumenExpenditureEdit form = new FrmAlterBitumenExpenditureEdit();
        ...
        {
            FrmAutomatismAlterBitumenExpenditure AutomatismExpenditure = new FrmAutomatismAlterBitumenExpenditure();
        ...
        private void btnDeleteExpenditure_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确定要删除吗？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
            {
                ...
                for (int i = 0; i < deleteIndex; i++)
                {
                    ...
                    {
                        deleteStatus1 = costDetailLogic.deleteAlterBitumenExpenditureLogic(Convert.ToInt64(dgvShow.SelectedRows[i].Cells["id"].Value.ToString()));
                    }
                    else
                    {
                        deleteStatus2 = costDetailLogic.deleteAlterBitumenWaitDetachExpenditureLogic(Convert.ToInt64(dgvShow.SelectedRows[i].Cells["id"].Value.ToString()));
                    }
                }
        ...
        private void FrmAlterBitumenCostCount_Load(object sender, EventArgs e)
        {
            this.userName = "fzh";
            #region// 初始化产品名称和设备名称
            // 产品名称和设备名称的初始化
            ...
            if (dgvShow.SelectedRows.Count > 0)
            {
                if (MessageBox.Show("是否确认审核选中记录", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    foreach (DataGridViewRow dgvr in dgvShow.SelectedRows)
                    {
                        ...
                        {
                            MessageBox.Show(auditingActionAlterBitumen.DoAssessor(long.Parse(dgvr.Cells["id"].Value.ToString()), this.UserName).ToString());
                            dgvr.Cells["审核人"].Value = this.UserName;
                        }
                        else
                        {
                            MessageBox.Show(auditingActionAlterBitumenDetach.DoAssessor(long.Parse(dgvr.Cells["id"].Value.ToString()), this.UserName).ToString());
                            dgvr.Cells["审核人"].Value = this.UserName;
                        }
                    }
                }
        ...
        private void btn_checkupMan_Click(object sender, EventArgs e)
        {
            if (dgvShow.SelectedRows.Count > 0)
            {
                ...
                if (frmConfirm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        int result = auditingActionAlterBitumen.DoConfirm(long.Parse(dgvShow.SelectedRows[0].Cells["id"].Value.ToString()), this.UserName, frmConfirm.Confirm);
                        switch (result)
                        {
                            case 0:
                    ...
                    {
                        int result = auditingActionAlterBitumenDetach.DoConfirm(long.Parse(dgvShow.SelectedRows[0].Cells["id"].Value.ToString()), this.UserName, frmConfirm.Confirm);
                        switch (result)
                        {
                            case 0:
                                if (frmConfirm.Confirm)
