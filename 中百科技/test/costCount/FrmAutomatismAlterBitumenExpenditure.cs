...
{
    public partial class FrmAutomatismAlterBitumenExpenditure : BaseForm
    {

        public FrmAutomatismAlterBitumenExpenditure()
        {
            InitializeComponent();
        }
            ...
            switch (cmbKind.Text.Trim())
            {
                case "直接人工":
                    ...
                    foreach (string name in fields)
                    {
                        ...
                        string hasAutoMoney = bitumenCLogic.isAutoAlterBitumenLogic(directManEntity);
        ...
        // 保存表中的所有内容 要根据产品是否已经定下来决定是存入各种费用表还是存入各种费用的待摊表
        private void button2_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count != 0)
            {
                ...
                saveSuccess = bitumenCLogic.saveAutoAlterExpenditure(dt);
                if (saveSuccess)
                {
                    ...
                    //foreach (DataGridViewRow dgvr in r)
                    //{
