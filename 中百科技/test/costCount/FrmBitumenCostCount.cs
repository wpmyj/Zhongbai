...
{
    public partial class FrmBitumenCostCount : BaseForm
    {

        private ArrayList treefields = new ArrayList();
        public FrmBitumenCostCount()
        {
            InitializeComponent();
        }
        AuditingAction auditingActionBitumen = new AuditingAction("id", "bitumenExpenditure");
        AuditingAction auditingActionBitumenDetach = new AuditingAction("id", "bitumenWaitDetachExpenditure");
        ...
        private void btnExpenditureAdd_Click(object sender, EventArgs e)
        {
            ...
            FrmBitumenExpenditureEdit form = new FrmBitumenExpenditureEdit();
        ...
        {
            FrmAutomatismBitumenExpenditure AutomatismExpenditure = new FrmAutomatismBitumenExpenditure();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (treeFieldForm.DialogResult == DialogResult.Yes)
            {
                ...
                if (fields.Count > 0)
                {
                    ...
                    logic.DistillBitumenExpenditureTree(ref treeView1, fields);
                }
            }
        ...
        private void FormInit()
        {
            ...
            CostEditLogic logic = new CostEditLogic();
            treeView1.Nodes.Clear();
            logic.DistillBitumenExpenditureTree(ref treeView1, treefields);
            viewStatus();
        }
        ...
        private void btnDeleteExpenditure_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确定要删除吗？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
            {
                ...
                for (int i = 0; i < deleteIndex; i++)
                {
                    ...
                    {
                        deleteStatus1 = costDetailLogic.deleteBitumenExpenditureLogic(Convert.ToInt64(dgvShow.SelectedRows[i].Cells["id"].Value.ToString()));
                    }
                    else
                    {
                        deleteStatus2 = costDetailLogic.deleteBitumenWaitDetachExpenditureLogic(Convert.ToInt64(dgvShow.SelectedRows[i].Cells["id"].Value.ToString()));
                    }
                }
        ...
        private void FrmBitumenCostCount_Load(object sender, EventArgs e)
        {
        ...
        {
            FrmCaleBitumenHeatCost caleBitumenHeatForm = new FrmCaleBitumenHeatCost();
            caleBitumenHeatForm.FormSort = 1;
            caleBitumenHeatForm.Text = "计算沥青加热成本";
            caleBitumenHeatForm.ShowDialog();
        }
            ...
            if (dgvShow.SelectedRows.Count > 0)
            {
                if (MessageBox.Show("是否确认审核选中记录", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    foreach (DataGridViewRow dgvr in dgvShow.SelectedRows)
                    {
                        ...
                        {
                            MessageBox.Show(auditingActionBitumen.DoAssessor(long.Parse(dgvr.Cells["id"].Value.ToString()), this.UserName).ToString());
                            dgvr.Cells["审核人"].Value = this.UserName;
                        }
                        else
                        {
                            MessageBox.Show(auditingActionBitumenDetach.DoAssessor(long.Parse(dgvr.Cells["id"].Value.ToString()), this.UserName).ToString());
                            dgvr.Cells["审核人"].Value = this.UserName;
                        }
                    }
                }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (dgvShow.SelectedRows.Count > 0)
            {
                ...
                if (frmConfirm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        int result = auditingActionBitumen.DoConfirm(long.Parse(dgvShow.SelectedRows[0].Cells["id"].Value.ToString()), this.UserName, frmConfirm.Confirm);
                        switch (result)
                        {
                            case 0:
                    ...
                    {
                        int result = auditingActionBitumenDetach.DoConfirm(long.Parse(dgvShow.SelectedRows[0].Cells["id"].Value.ToString()), this.UserName, frmConfirm.Confirm);
                        switch (result)
                        {
                            case 0:
                                if (frmConfirm.Confirm)
