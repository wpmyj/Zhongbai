...
{
    public partial class FrmTransportContractadd : common.BaseForm 
    {
        public FrmTransportContractadd()
        {
            InitializeComponent();
        }
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            if (formHelper.inputCheck(groupBox1))
            {
                if (checkContract())
                {
                    MessageBox.Show("合同名称或合同编号或补充合同编号已存在", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }               
        ...
        private bool checkContract()
        {
            ...
            {
                return transportLogic.AlonekContract(txtno.Text.Trim(),txt_name.Text.Trim());
            }
            else
            {
        ...
        private TransportContract getTransportContract()
        {
            ...
            if (transportLogic.getcType(rbcType1, rbcType2) == 0)
            {
                ...
                transportContract.no =txtno.Text.Trim();
            }
            else
            {
        ...
        private void rbcType_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {          
                ...
                cmb_transportUnit.Enabled = true;
                cmb_transportUnit.SelectedIndex = -1;
            }
            else
            {
                cmb_no.Visible = true;
        ...
        // 2009-4-20 修改人：付中华  合同编号选中之后相应的运输单位显示 不可编辑
        private void cmb_no_SelectionChangeCommitted(object sender, EventArgs e)
        {
            cmb_transportUnit.Enabled = true;
            if (cmb_no.SelectedValue.ToString() != "")
            {
                cmb_transportUnit.Text = transportLogic.findConveyAncer(Convert.ToInt32(cmb_no.SelectedValue.ToString())).Rows[0][0].ToString();
            }
            cmb_transportUnit.Enabled = false;
